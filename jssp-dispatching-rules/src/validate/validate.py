...
def validate_instance(durations: np.ndarray, affectations: np.ndarray):
    ...
    assert (
        durations.shape == affectations.shape
    ), f"Wrong number of jobs or machines (found {durations.shape} and {affectations.shape})"
    assert (
...
def validate_job_tasks(
    ...
    assert np.all(
        schedule >= previous_ending_times
...
def validate_machine_tasks(
    ...
    for machine_id in range(n_machines):
        ...
        assert np.all(
            schedule_machine >= previous_ending_times
